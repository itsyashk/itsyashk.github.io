...
export const projects: Project[] = [
    ...
    {
        slug: "shadow-hand-grasping",
        title: "Humanoid Hand Grasping Simulator",
        categories: ["Robotics", "Algorithms"],
        tags: ["Grasping", "MuJoCo", "Force Closure", "Manipulation", "Simulation", "Shadow Hand"],
        stack: ["Python", "MuJoCo", "NumPy", "SciPy", "Matplotlib"],
        date: "2026",
        summary: "Quasi-static grasp simulator and analyzer for the Shadow Hand E3M5, computing force closure and grasp-quality metrics across five free rigid objects in MuJoCo.",
        description: "We built a MuJoCo simulation of a humanoid multi-fingered robotic hand grasping several free rigid objects, using the Shadow Hand E3M5 model from MuJoCo Menagerie. It is a fully articulated hand with a palm, four fingers, an opposable thumb, joint limits, actuators, collision geometry, and real contact and friction handling. The goal was a setup that behaves like a real robotic grasp rather than a toy gripper. Each target (a sphere, a box, an upright cylinder, a mug with a handle, and an irregular rock) is a free rigid body with mass, inertia, friction, and a free joint, so a bad grasp lets the object slip or fall instead of staying glued in place. The hand root is commanded like a robot wrist, and every grasp runs the same quasi-static sequence: open, preshape near the object, gradually close the fingers, settle under contact and friction, then lift while holding the grasp command. After the lift we check whether the object is still held by valid finger contacts.",
        approach: "MuJoCo reports every collision contact, so we first filter down to meaningful grasp contacts in the final held state: contacts between the object and a named digit link, with positive normal force, excluding palm contacts and rejecting deep soft-constraint penetration artifacts. For each accepted contact we form the object wrench w = [f; r × f] in R⁶, sampling multiple force directions from the friction cone and stacking them into a grasp wrench set. The wrench rank is the numerical rank of that set; force closure is estimated by checking whether the origin lies inside the convex hull of the available wrenches; and the epsilon-style quality is the smallest signed distance from the origin to a hull facet, so larger values mean a more robust grasp. All five objects reached force closure with a full-rank (6-D) wrench space and were lifted with valid digit contacts. The box and rock grasps scored highest (epsilon ~0.23 and ~0.16); the narrow side-grasp on the cylinder scored lowest (~0.0002), which we kept in the results as an honest signal of where the simulator is weakest.",
        metrics: ["5/5|Objects Force-Closure Grasped & Lifted", "6-D|Full-Rank Grasp Wrench Space", "0.0002–0.23|Epsilon Grasp-Quality Range"],
        images: [
            "/images/projects/hand-grasp.gif",
            "/images/projects/grasp-box.png",
            "/images/projects/grasp-rock.png",
            "/images/projects/grasp-mug.png",
        ],
    }
];
